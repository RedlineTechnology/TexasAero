...
if ( ! function_exists( 'ta_setup' ) ) :
	...
	function ta_setup() {
		 ...
		 * to change 'ta' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'ta', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'ta' ),
			'menu-2' => esc_html__( 'Secondary', 'ta' ),
			'menu-3' => esc_html__( 'Footer', 'ta')
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'ta_custom_background_args', array(
...
function ta_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'ta' ),
		'id'            => 'sidebar',
		'description'   => esc_html__( 'Add widgets here.', 'ta' ),
...
function ta_scripts() {
	...
	wp_enqueue_script( 'ta-navigation', get_template_directory_uri() . '/js/navigation.min.js', array('jquery'), '20190220', true );
	// This thing
	wp_enqueue_script( 'ta-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
	// Slick
...
add_action( 'wp_enqueue_scripts', 'ta_scripts' );
